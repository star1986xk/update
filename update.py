...
class downwin_class(object):
    def __init__(self):
        # 下载新版本zip的地址
        self.url_down = 'http://soft.sanmoo.com:8080/update/dig_word_update.zip'
        # 创建主窗口
        self.window = tk.Tk()
        # 设置标题
        ...
        self.window.update()
        if self.usr_download(self.url_down):
            zfile = zipfile.ZipFile("./dig_word_update.zip", "r")
            zfile.extractall()
            zfile.close()
            os.remove('./dig_word_update.zip')
    ...
    def usr_download(self, url):
        try:
            if os.path.exists('./dig_word_update.zip'):
                os.remove('./dig_word_update.zip')
            # 点击按钮后把按钮设置为不可用
